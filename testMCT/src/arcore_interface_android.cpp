...
bool ARCoreInterface::_initialize() {
    ...
    if (init_status == INITIALISE_FAILED) {
        ...
        std::cout<<"Godot ARCore: Initialising..."<<std::endl;
        init_status = START_INITIALISE;

        // create our camera feed
        if (feed.is_null()) {
            std::cout<<"Godot ARCore: Creating camera feed..."<<std::endl;
            ...
            if (cs != nullptr) {
                ...
                std::cout<<"Godot ARCore: Feed "<<feed->get_id()<<" added"<<std::endl;
            }
        }
...
void ARCoreInterface::_process() {
    ...
    if (new_display_rotation != display_rotation) {
        ...
        if ((display_rotation == 1) || (display_rotation == 3)) {
            ArSession_setDisplayGeometry(m_pimpl->ar_session, display_rotation, height, width);
        } else {
            ArSession_setDisplayGeometry(m_pimpl->ar_session, display_rotation, width, height);
        }
